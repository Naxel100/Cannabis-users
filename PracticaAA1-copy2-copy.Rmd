...
```{r}
summary(dd[, c(2:13, 29, 18)])
```

### 1.2 Boxplots

```{r}
par(mfrow = c(2, 2), mex = 0.55)

for (i in 6:12)
   boxplot(dd[, i] ~ dd$Cannabis, main = colnames(dd)[i], xlab = "", ylab = "", 
           col = 8, horizontal = T)
```

### 1.3 Barplots i graficació de taules de contingència

```{r}
par(mfrow = c(1, 2), mex = 0.4)
for(i in c(2:5, 13, 29)) {
   tab =  table(dd$Cannabis, dd2[,i])
...
### 1.5 Anàlisi de correspondències
...
### 1.6 Test de wilcox per les mitjanes dels scores
...
Normalitat per l'ús de Shapiro-Wilk
...
### 1.7 Estudi normalitat per a l'ús de QDA i LDA

Normalitat variable per variable
...
train = dd[samples, c(2:12, 28, 17)]
train2 = dd2[samples, c(2:12, 28, 17)]
test = dd[-samples, c(2:12, 28, 17)]
test2 = dd2[samples, c(2:12, 28, 17)]
...
```{r}
train.short = train[,-(11:13)]
pca = princomp(train.short, cor = T)
A = pca$loadings
train.pca = as.data.frame(pca$scores)
train.pca[c("Alcohol", "Nicotine", "Cannabis")] = train[, 11:13]
```
...
(size = model.10x10CV$bestTune$size)
```

```{r}
decays = seq(0, 1, by = 0.1)

model.10x10CV = train(Cannabis ~ ., data = train.pca, 
                      method = 'nnet', maxit = 1000, trace = FALSE,
                      tuneGrid = expand.grid(.size = 1, .decay = decays), trControl = trc)
...
Aquesta funció genera un model lineal generlatizat binomial amb link l'nidicat al paràmetre d'entrada *linkage* amb les dades d'entrenament i retorna l'error comés sobre el conjunt de dades de validació. De nou incloem un *flag* que ens indica si volem fer el print de taula de confusió o no.
...
for (k in ntrees) {
   ...
   for (i in 1:1) {
      ...
      for (j in 1:1) {
         ...
         yl = train[samples[-(start:end)], droga]
         Xv = train[samples[start:end], -13]
         yv = train[samples[start:end], droga]
   ...
   print(k) 
   tr.error[cont] = err["training"]/9
   te.error[cont] = err["test"]/9
   cont = cont + 1
}

ggplot(ntrees, tr.error, ylab = "error", col = "dark green", type = "l", ylim=c(0, 0.3))
points(ntrees, te.error, type = "l", col = "blue")
```
```{r}
#ggplot(ntrees, tr.error, ylab = "error", col = "dark green", type = "l", ylim=c(0, 0.3))
#points(ntrees, te.error, type = "l", col = "blue")
...
Fem un 10 times 10-fold cross validation per avaluar quin dels models proposats és el millor. En aquest cas hem usat *QDA*, *LDA*, *GLM link logit*, *GLM link probit*, *GLM link loglog* i *Naive Bayes*.

```{r, warning=F}
set.seed(6969)
droga = "Cannabis"  # Indiquem el nom de la droga a estudi
...
for (i in 1:4) {
   ...
   for (j in 1:4) {
      ...
      #error["QDA"] = error["QDA"] + error_qda(Xl[-(11:12)], yl, Xv[-(11:12)], yv, F)
      #error["LDA"] = error["LDA"] + error_lda(Xl[-(11:12)], yl, Xv[-(11:12)], yv, F)
      ...
      error["Nnet"] = error["Nnet"] + error_nnet(Xl, yl, Xv, yv, size, decay, F)
      error["Random Forest"] = error["Random Forest"] + error_randomforest(Xl, yl, Xv, yv, 50, F)
   }
}
error/16
```
...
for (i in 1:4) {
   ...
   for (j in 1:4) {
      ...
      # Separem en variables explicatives i resposta / learn i validation
      Xl = train2[samples[-(start:end)], -13]
      yl = train2[samples[-(start:end)], droga]
      Xv = train2[samples[start:end], -13]
      yv = train2[samples[start:end], droga]
...
### 2.8 Error sobre les dades de test amb model glm, link logit

Considerant els resultats obtinguts en *Cross-validation* podem observar que el millor model per a la predicció és usant un model lineal generalitzat binomial amb link *logit*. Per tant, ara, podem entrenar un model amb totes les dades de train i veure l'error que es produeix en la predicció sobre les dades de test.

```{r}
test.pca = as.data.frame(as.matrix(test[1:10]) %*% A)
test.pca[c("Alcohol", "Nicotine", "Cannabis")] = test[, 11:13]

print(error_nnet(train.pca[, -13], train.pca[, droga], test.pca[, -13], test.pca[, droga], size, decay, T))
print(error_glm(train.pca[, -13], train.pca[, droga], test.pca[, -13], test.pca[, droga], "logit", T))
print(error_naivebayes(train.pca[, -13], train.pca[, droga], test.pca[, -13], test.pca[, droga], T))
```

Observem que és un error similar a l'error produït sobre les mateixes dades d'entrenament:

```{r}
print(error_glm(train[, -13], train[, droga], train[, -13], train[, droga], "logit", T))
```
